...
interface SpeedFinderToolProps {
  /** Still-live, not-crossed-out candidates — only these can be highlighted as matches. */
  candidates: Ghost[];
  onHighlightMatches: (ids: string[]) => void;
}

/** Tap along with the ghost's hunt footsteps; taps more than 2.5s apart start a fresh session. */
...
export function SpeedFinderTool({ candidates, onHighlightMatches }: SpeedFinderToolProps) {
  ...
  const matches = useMemo(() => {
    ...
    return candidates.filter((g) =>
      g.hunt.speeds.some((s) => Math.abs(s.value - estimate.speed) <= tol)
    );
  }, [estimate, candidates]);
      ...
      <div className="flex flex-1 flex-col gap-2 rounded-lg border border-surface-border bg-surface-2/50 p-2.5">
        ...
        {estimate ? (
          ...
          <p className="text-[11px] text-muted">
            Tap in rhythm with its footsteps during a hunt — needs at least 2 taps to estimate a speed.
          </p>
        )}
      </div>
    </div>
